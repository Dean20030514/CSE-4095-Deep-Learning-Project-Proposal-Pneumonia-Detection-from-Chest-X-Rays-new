...
**Problem.** Pneumonia remains a major global health burden; timely triage reduces risk of missed cases. This project builds a **binary classifier** (NORMAL vs PNEUMONIA) for chest X‑rays to support **educational research** and demo‑level decision support.

**Current Implementation Status:** The project currently implements binary classification with 1,341 NORMAL and 3,875 PNEUMONIA training images. The codebase supports standard deep learning workflows including training, evaluation, and interactive demo via Streamlit.
...
- **W7** _(optional)_: External validation and robustness tests (blur/noise/contrast curves).
- **W8–W9**: Report, poster, and demo video; finalize model card.
- **W10** _(buffer)_: Repro pass; packaging (ONNX/Docker if Track‑B).
...
## 6) Label Strategy (Binary Classification)

**Current Implementation:** The project uses a **binary classification** approach with two classes:
- **NORMAL**: 1,341 training images (25.8%)
- **PNEUMONIA**: 3,875 training images (74.2%)

This represents a significant class imbalance that must be addressed through:
- Weighted Cross-Entropy loss or Focal Loss
- WeightedRandomSampler for balanced mini-batches (already implemented)
- Careful monitoring of per-class recall metrics

**Future Extension:** The codebase can be extended to tri-class (Normal/Bacterial/Viral) if datasets with finer-grained labels become available.
...
cd CSE-4095-Deep-Learning-Project-Proposal-Pneumonia-Detection-from-Chest-X-Rays-new-main

# 2) Create environment (pick one):
# Option A: Conda
conda env create -f environment.yml && conda activate cxr

# Option B: Pip (Windows PowerShell)
python -m venv .venv
...
# 5) Run the demo app in demo_mode (no weights needed)
streamlit run src/app/streamlit_app.py -- --demo_mode
```

**What you should see (≤30 min):** Streamlit UI loads, accepts an image upload, outputs class probabilities + Grad‑CAM overlay (in demo mode, shows static example).
...
The following scripts are already implemented and ready to use:
...
**`scripts/demo_presentation.py`** - Auto-generates presentation assets including metrics plots, confusion matrices, CAM grids, and threshold curves.
...
- 1:15–2:00 Thresholding: _Max‑Recall_ vs _Balanced_ modes
...
**Metrics**: overall & per‑class P/R/F1, macro‑F1, PR‑AUC; **thresholded modes** (_max‑recall_ & _balanced_); calibration.
